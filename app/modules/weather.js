/* Milestone 5 — weather module. Open-Meteo (free, no key), poll-and-diff:
 * fetch current conditions every 10 min per city panel, derive a coarse state
 * (clear/cloudy/fog/rain/snow/storm), and only touch the DOM when it changes.
 * Overlay animation periods all divide ATLAS_CONFIG.loopSeconds (60 s).
 * Debug: ?wx=rain|snow|storm|fog|cloudy|clear forces every panel. */
(function () {
  "use strict";
  ...
  function classify(code) {
    if (code === 0) return { state: "clear", intensity: 0 };
    ...
    if (code <= 55) return { state: "rain", intensity: 0.35 }; // drizzle
    if (code <= 57) return { state: "rain", intensity: 0.5 };
    if (code <= 63) return { state: "rain", intensity: 0.6 };
    if (code <= 65) return { state: "rain", intensity: 1 };
    if (code <= 67) return { state: "snow", intensity: 0.6 }; // freezing rain ~ sleet
    if (code <= 75) return { state: "snow", intensity: code === 71 ? 0.4 : 0.8 };
    if (code === 77) return { state: "snow", intensity: 0.5 };
    if (code <= 82) return { state: "rain", intensity: 0.8 }; // showers
    if (code <= 86) return { state: "snow", intensity: 0.9 };
  ...
  const mod = {
    ...
    start(atlas) {
      for (const p of atlas.panels) {
        ...
        el.innerHTML = '<div class="wx-rain"></div><div class="wx-snow"></div><div class="wx-fog"></div><div class="wx-flash"></div>';
        const readout = document.createElement("div");
        readout.className = "wx-readout";
        el.appendChild(readout);
        p.el.insertBefore(el, p.tint); // under the night tint
        this.panels.push({ panel: p, mapId, lon: map.center[0], lat: map.center[1], el, readout, state: null, temp: null });
      }
      if (!this.panels.length) return;
      const poll = () => this.pollAll(atlas);
      poll();
    ...
    async pollAll() {
      for (const c of this.panels) {
        try {
          if (forced) { this.apply(c, { state: forced === "clear" ? "clear" : forced, intensity: 0.8 }, 21, true); continue; }
          ...
          this.apply(c, classify(cur.weather_code), cur.temperature_2m, cur.is_day);
        } catch (e) {
